...
def evaluate_metrics(signals_df):
    ...
    portfolio_evaluation_df.loc["Annual Volatility"] = (1 + signals_df["Portfolio Daily Returns"].std() * np.sqrt(252))
...
def main():
    """Main Event Loop."""
    # Get me the data - AAPL csv
    data_df = fetch_data()
    
    # Pass data_df into generate_signals
    signals_df = generate_signals(data_df)
    
    # Get your signals and test with a initial cap, and no. of shares
